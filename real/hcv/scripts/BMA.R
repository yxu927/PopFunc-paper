...
# 1) Load or install tidyverse if not already installed
# install.packages("tidyverse")
library(tidyverse)

# 2) Read the log file (tab-delimited, ignoring lines starting with '#')
#    Adjust the file path as needed
log_file_path <- "../beast/hcv.log"
df <- read_tsv(log_file_path, comment = "#")

# 3) Examine column names to confirm that I and I_na are present
colnames(df)

# (Optional) Check total rows = MCMC samples
n_total <- nrow(df)
print(paste("Total MCMC samples (including burn-in) =", n_total))

# 4) Define burn-in fraction (e.g., 10%)
burnin_frac <- 0.1
start_idx <- floor(n_total * burnin_frac)

# Discard the first 10% of iterations, keep the remaining 90%
df_post <- df %>%
  slice((start_idx + 1):n_total)

# 5) Map (I, I_na) to model names
map_model <- function(I_val, I_na_val) {
  dplyr::case_when(
    I_val == 0 & I_na_val == 0 ~ "Exponential",
    I_val == 0 & I_na_val == 1 ~ "Exponential Expansion",
    I_val == 1 & I_na_val == 0 ~ "Logistic",
    I_val == 1 & I_na_val == 1 ~ "Logistic Expansion",
    I_val == 2 & I_na_val == 0 ~ "Gompertz",
    I_val == 2 & I_na_val == 1 ~ "Gompertz Expansion",
    I_val == 3 & I_na_val == 0 ~ "Exponential to constant",
    I_val == 3 & I_na_val == 1 ~ "Exp. expansion to constant",
    I_val == 4 ~ "C–E–C",
    TRUE ~ "Unknown"
  )
}

# 6) Tabulate each model's posterior frequency
...
df_posterior <- df_posterior %>%
  mutate(
    logBF = if_else(
      Posterior > 0,
      log(Posterior / best_model_posterior),
      -Inf
    )
  )

# Final output: posterior probabilities and logBF
df_posterior
